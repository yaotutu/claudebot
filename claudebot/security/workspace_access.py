...
def workspace_sandbox_status(
    *,
    ...
    return WorkspaceSandboxStatus(
        ...
        provider="none",
        provider_label=_provider_label("none"),
        summary="Workspace restriction uses claudebot application-level guards.",
    )
...
def _env_system_provider(environ: dict[str, str] | None = None) -> str | None:
    ...
    explicit_provider = env.get("CLAUDEBOT_WORKSPACE_SANDBOX_PROVIDER")
    enforced = env.get("CLAUDEBOT_WORKSPACE_SANDBOX_ENFORCED")
    compatibility = env.get("CLAUDEBOT_SANDBOX_ENFORCED")
