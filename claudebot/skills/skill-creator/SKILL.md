...
For `claudebot`, custom skills should live under the active workspace `.claude/skills/` directory so Claude Code can discover them as native skills (for example, `<workspace>/.claude/skills/my-skill/SKILL.md`).
...
Keep frontmatter minimal. In `claudebot`, `metadata` and `always` are also supported when needed, but avoid adding extra fields unless they are actually required.
