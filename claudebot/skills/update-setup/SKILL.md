---
name: update-setup
description: One-time setup wizard for the claudebot upgrade skill. Triggers: setup update, configure update, 切设置更新, 初始化更新.
---
...
Use `exec` to run `claudebot --version`. Tell the user the current version.

Then collect install clues with `exec`. These commands are best-effort; if one fails,
keep going and show the useful output:

```
command -v claudebot || true
python -m pip show claudebot || true
pipx list | sed -n '/claudebot/,+3p' || true
uv tool list | sed -n '/claudebot/,+3p' || true
```
...
answer, stop and ask the user to rerun this setup when they know how claudebot was
installed.
...
`question: "Where is your claudebot source checkout? Enter an absolute path or a path relative to this workspace:"`.

**Question 2 — Optional dependencies:**

```
question: "Which optional dependencies do you need? List names separated by spaces, or reply 'none'. Available: langsmith, pdf"
```
...
| Method | Command |
|--------|---------|
| uv | `uv tool install "claudebot[EXTRAS]" --force` |
| pipx | `pipx install --force "claudebot[EXTRAS]"` |
| pip | `python -m pip install --upgrade "claudebot[EXTRAS]"` |
...
---
name: update
description: "Upgrade claudebot to the latest version. Triggers: upgrade claudebot, update claudebot, 升级claudebot, 更新claudebot."
---
...
4. Use `exec` to verify: `claudebot --version`
5. Tell the user the new version. Say: "Run `/restart` to restart claudebot and apply the update. If `/restart` is unavailable in this channel, restart the claudebot process manually."
```

## Step 5: Confirm

Tell the user: "Upgrade skill created. Say 'upgrade claudebot' when you want to update."
