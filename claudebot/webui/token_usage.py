...
from claudebot.config.paths import get_webui_dir
...
_REQUEST_KEYS = ("requests", "runtime_requests", "estimated_requests")
...
def _row_int(row: dict[str, Any], key: str) -> int:
    if key == "runtime_tokens":
        return _clean_int(row.get("runtime_tokens", row.get("provider_tokens")))
    if key == "runtime_requests":
        return _clean_int(row.get("runtime_requests", row.get("provider_requests")))
    return _clean_int(row.get(key))
...
def _normalize_usage(raw: dict[str, Any] | None) -> dict[str, int]:
    if not isinstance(raw, dict):
        return {}
    usage = {key: _row_int(raw, key) for key in _USAGE_KEYS}
    fallback_total = usage["prompt_tokens"] + usage["completion_tokens"]
    if usage["total_tokens"] <= 0:
        usage["total_tokens"] = fallback_total
    if usage["estimated_tokens"] <= 0 and usage["runtime_tokens"] <= 0:
        usage["runtime_tokens"] = usage["total_tokens"]
    elif usage["estimated_tokens"] > 0 and usage["runtime_tokens"] <= 0:
        usage["estimated_tokens"] = min(usage["estimated_tokens"], usage["total_tokens"])
    elif usage["runtime_tokens"] > 0 and usage["estimated_tokens"] <= 0:
        usage["runtime_tokens"] = min(usage["runtime_tokens"], usage["total_tokens"])
    return usage if usage["total_tokens"] > 0 else {}


def _normalize_usage_row(row: dict[str, Any]) -> dict[str, int]:
    cleaned = {key: _row_int(row, key) for key in _USAGE_KEYS}
    if cleaned["total_tokens"] <= 0:
        cleaned["total_tokens"] = cleaned["prompt_tokens"] + cleaned["completion_tokens"]
    if cleaned["runtime_tokens"] <= 0 and cleaned["estimated_tokens"] <= 0:
        cleaned["runtime_tokens"] = cleaned["total_tokens"]
    requests = {key: _row_int(row, key) for key in _REQUEST_KEYS}
    if (
        requests["requests"] > 0
        and requests["runtime_requests"] <= 0
        and requests["estimated_requests"] <= 0
    ):
        if cleaned["estimated_tokens"] > 0 and cleaned["runtime_tokens"] <= 0:
            requests["estimated_requests"] = requests["requests"]
        else:
            requests["runtime_requests"] = requests["requests"]
...
def record_token_usage(
    ...
    *,
    ...
    with _WRITE_LOCK:
        ...
        if normalized.get("estimated_tokens", 0) > 0 and normalized.get("runtime_tokens", 0) <= 0:
            row["estimated_requests"] = _clean_int(row.get("estimated_requests")) + 1
        else:
            row["runtime_requests"] = _clean_int(row.get("runtime_requests")) + 1
        ...
        if normalized.get("estimated_tokens", 0) > 0 and normalized.get("runtime_tokens", 0) <= 0:
            ...
            source_row["runtime_requests"] = _clean_int(source_row.get("runtime_requests")) + 1
...
class TokenUsageHook:
    """Compatibility no-op for the removed legacy AgentHook integration."""
    ...
    async def after_iteration(self, context: Any) -> None:
        try:
            record_token_usage(
                getattr(context, "usage", None),
                source=_source_from_session_key(getattr(context, "session_key", None)),
