...
from claudebot.apps.protocol import app_manifest, compact_dict
from claudebot.config.paths import get_runtime_subdir
from claudebot.security.workspace_policy import is_path_within
...
CLAUDEBOT_EXTENSION_REGISTRY_URL = (
    "https://raw.githubusercontent.com/claudebot/claudebot-extension/main/registry.json"
)
CLAUDEBOT_EXTENSION_RAW_BASE = (
    "https://raw.githubusercontent.com/claudebot/claudebot-extension/main"
)
_CATALOG_SOURCES = (
    ...
    ("extensions", CLAUDEBOT_EXTENSION_REGISTRY_URL, CLAUDEBOT_EXTENSION_RAW_BASE, False),
)
...
class CliAppManager:
        ...
        remove = compact_dict(
            {
                ...
                "verification": (
                    ...
                    else ["claudebot_state_absent", "managed_paths_absent"]
                ),
            }
        )
        return app_manifest(
    ...
    def _fallback_skill(self, app: dict[str, Any]) -> str:
        ...
        description = _catalog_description(app) or f"Use {display} from claudebot."
        return f"""---
...
Use this skill when the user asks claudebot to operate {display} through its installed CLI app.
...
"""
    ...
    def _with_claudebot_skill_note(self, content: str, app: dict[str, Any]) -> str:
        marker = "<!-- claudebot-cli-app-note -->"
...
"""
    ...
    def install_skill(self, app: dict[str, Any]) -> Path:
        ...
        content = self._with_claudebot_skill_note(content, app)
    ...
    def uninstall(self, name: str) -> dict[str, Any]:
        ...
        if strategy != "bundled":
            ...
            if still_managed or (not managed_entry_path and still_available):
                ...
                message = (
                    ...
                    "so claudebot kept it installed."
                )
                return self.payload() | {
                    "last_action": {
                        "ok": False,
        ...
        if strategy == "bundled" and still_available:
            message = (
                f"Removed {app['display_name']} from claudebot. {entry_point} "
                "is still available because it is managed outside claudebot."
            )
