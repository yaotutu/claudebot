...
class SkillsLoader:
    ...
    def __init__(
        ...
        self.workspace_skills = workspace / ".claude" / "skills"
    ...
    def _skill_entries_from_dir(
        ...
        for skill_dir in base.iterdir():
            ...
            if skip_names is not None and name in skip_names:
                continue
            actual_source = "builtin" if name.startswith("builtin-") else source
            entries.append({"name": name, "path": str(skill_file), "source": actual_source})
        return entries
    ...
    def list_skills(self, filter_unavailable: bool = True) -> list[dict[str, str]]:
        ...
        if self.disabled_skills:
            disabled = self.disabled_skills | {f"builtin-{name}" for name in self.disabled_skills}
    ...
    def _parse_claudebot_metadata(self, raw: object) -> dict:
        """Extract claudebot/openclaw metadata from a frontmatter field.
        ...
        if not isinstance(data, dict):
            return {}
        payload = data.get("claudebot", data.get("openclaw", {}))
    ...
    def _get_skill_meta(self, name: str) -> dict:
        """Get claudebot metadata for a skill (cached in frontmatter)."""
        raw_meta = self.get_skill_metadata(name) or {}
        return self._parse_claudebot_metadata(raw_meta.get("metadata"))
    ...
    def get_always_skills(self) -> list[str]:
        ...
        return [
            ...
            and (
                self._parse_claudebot_metadata(meta.get("metadata")).get("always")
                or meta.get("always")
            )
        ]
