...
- If the app CLI is missing, lacks local app/API prerequisites, or cannot complete the requested action, explain that concrete blocker and what was attempted.
...
- Use `cron` for scheduled reminders or recurring jobs; do not run `claudebot cron` through `exec`.
