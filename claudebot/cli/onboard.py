"""Interactive onboarding questionnaire for claudebot."""
...
def _show_main_menu_header() -> None:
    ...
    from claudebot import __logo__, __version__

    console.print()
    # Use Align.CENTER for the single line of text
    from rich.align import Align

    console.print(Align.center(f"{__logo__} [bold cyan]claudebot[{__version__}][/bold cyan]"))
    console.print()
...
def _input_context_window(display_name: str, current: Any, model_obj: BaseModel) -> int | None:
    """Get context window input."""
...
def _handle_context_window_field(
    ...
    """Handle context_window_tokens with numeric validation."""
    new_value = _input_context_window(field_display, current_value, working_model)
...
def _get_channel_info() -> dict[str, tuple[str, type[BaseModel]]]:
    """External channel registry is removed in Claude Code gateway mode."""
    return {}
...
_SETTINGS_SECTIONS: dict[str, tuple[str, str, set[str] | None]] = {
    "Claude Code": ("Claude Code", "Configure Claude Code SDK gateway model and auth", None),
    "Agent Settings": ("Agent Defaults", "Configure agent runtime behavior", None),
    "Channel Common": (
        "Channel Common",
...
_SETTINGS_GETTER = {
    "Claude Code": lambda c: c.claude_code,
...
_SETTINGS_SETTER = {
    "Claude Code": lambda c, v: setattr(c, "claude_code", v),
...
def _show_summary(config: Config) -> None:
    ...
    _print_summary_panel(_summarize_model(config.claude_code), "Claude Code")
...
def run_onboard(initial_config: Config | None = None) -> OnboardResult:
    ...
    while True:
        ...
        _menu_dispatch = {
            "[L] Claude Code": lambda: _configure_general_settings(config, "Claude Code"),
