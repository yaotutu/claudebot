...
class IncrementalThinkExtractor:
    ...
    Streaming runtimes may expose only a single content delta channel. When a
...
def extract_reasoning(
    ...
    1. Dedicated ``reasoning_content`` from compatible model responses.
    2. Anthropic-style ``thinking_blocks``.
...
_TOOL_RESULTS_DIR = ".claudebot/tool-results"
...
def estimate_prompt_tokens(
    ...
    Counts all fields sent to the LLM: content, tool_calls,
...
def build_status_content(
    *,
    ...
    lines = [
        f"\U0001f408 claudebot v{version}",
