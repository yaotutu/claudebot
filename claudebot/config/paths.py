...
from claudebot.utils.helpers import ensure_dir


def get_config_path() -> Path:
    """Get the configuration file path (lazy import to break circular dependency).

    Delegates to ``claudebot.config.loader.get_config_path`` at call time so
    that importing this module never triggers a circular import during startup.
    """
    from claudebot.config.loader import get_config_path as _loader_get_config_path
...
def is_default_workspace(workspace: str | Path | None) -> bool:
    """Return whether a workspace resolves to claudebot's default workspace path."""
    default = get_config_path().parent / "workspace"
    current = Path(workspace).expanduser() if workspace else default
    return current.resolve(strict=False) == default.resolve(strict=False)


def get_cli_history_path() -> Path:
    """Return the shared CLI history file path."""
    return Path.home() / CONFIG_DIR_NAME / "history" / "cli_history"


def get_legacy_sessions_dir() -> Path:
    """Return the legacy global session directory used for migration fallback."""
    return Path.home() / CONFIG_DIR_NAME / "sessions"
