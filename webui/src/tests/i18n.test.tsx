...
describe("webui i18n", () => {
  ...
  it("switches UI copy and document locale through the language switcher", async () => {
    ...
    expect(localStorage.getItem("claudebot.locale")).toBe("zh-CN");
  ...
  it("keeps Simplified Chinese settings copy localized", () => {
    ...
    expect(settings.nav.overview).toBe("Claude 网关");
    expect(settings.nav.appearance).toBe("外观");
    expect(settings.help.brandLogos).toBe("在设置中显示 Claude Code 和 CLI 标志。");
  });
});
