...
describe("ThreadComposer", () => {
  ...
  it("keeps the thread composer compact while matching the hero style", () => {
    render(
      <ThreadComposer
        onSend={vi.fn()}
        modelLabel="glm-cn/glm-5.1"
        placeholder="Type your message..."
      />,
    );

    expect(screen.getByText("glm-cn/glm-5.1")).toBeInTheDocument();
    expect(screen.getByTestId("composer-model-logo-claude-code")).toBeInTheDocument();
  ...
  it("keeps project selection as a compact composer dropdown", async () => {
    const onWorkspaceScopeChange = vi.fn();
    const defaultScope = {
      project_path: "/Users/test/.claudebot/workspace",
  ...
  it("uses the web path menu for project selection", async () => {
    const defaultScope = {
      project_path: "/Users/test/.claudebot/workspace",
      project_name: "workspace",
  ...
  it("renders slash commands as direct actions", () => {
    render(
      <ThreadComposer
        onSend={vi.fn()}
        placeholder="Type your message..."
    ...
    expect(screen.getByRole("option", { name: /Show help/i })).toBeInTheDocument();
  ...
  it("prioritizes stop as an immediate slash action while streaming", () => {
    ...
    expect(window.localStorage.getItem("claudebot.webui.slashCommandRecents")).toBeNull();
  });

  it("orders recent slash commands first for the blank slash menu", () => {
    window.localStorage.setItem("claudebot.webui.slashCommandRecents", JSON.stringify(["/help"]));
    render(
      <ThreadComposer
        onSend={vi.fn()}
        placeholder="Type your message..."
  ...
  it("sends plain messages without extra send options", () => {
    const onSend = vi.fn();
    render(
      <ThreadComposer
        onSend={onSend}
