  ...
  it("groups WebUI chats by workspace project while preserving in-project sorting and activity", () => {
    const sessions = [
      session({
        ...
        workspaceScope: {
          project_path: "/Users/me/claudebot",
          project_name: "claudebot",
      ...
      session({
        ...
        workspaceScope: {
          project_path: "/Users/me/claudebot-bench",
          project_name: "claudebot-bench",
          access_mode: "full",
        },
      }),
    ];
    ...
    const claudebotSection = screen.getByRole("region", { name: "claudebot" });
    const claudebotText = claudebotSection.textContent ?? "";

    expect(screen.getByRole("region", { name: "claudebot-bench" })).toBeInTheDocument();
    expect(within(claudebotSection).getByText("Alpha task")).toBeInTheDocument();
    expect(within(claudebotSection).getByText("Zeta task")).toBeInTheDocument();
    expect(claudebotText.indexOf("Alpha task")).toBeLessThan(claudebotText.indexOf("Zeta task"));
    expect(within(claudebotSection).getByLabelText("Agent running")).toBeInTheDocument();
  ...
  it("keeps default workspace chats in the Chats section instead of a project folder", () => {
    const sessions = [
      session({
        ...
        workspaceScope: {
          project_path: "/Users/me/.claudebot/workspace",
    ...
    render(
      <ChatList
        ...
        onRequestRename={vi.fn()}
        onToggleArchive={vi.fn()}
        defaultWorkspacePath="/Users/me/.claudebot/workspace"
        showTimestamps
      />,
    );

    expect(screen.getByText("Projects")).toBeInTheDocument();
    expect(screen.getByRole("region", { name: "claudebot" })).toBeInTheDocument();
  ...
  it("can collapse a project group and keeps project rename separate from chat titles", async () => {
    ...
    render(
      <ChatList
        ...
        projectNameOverrides={{ "/Users/me/claudebot": "Photos" }}
        collapsedGroups={{ "project:/Users/me/claudebot": true }}
      />,
    );
    ...
    expect(onToggleGroup).toHaveBeenCalledWith("project:/Users/me/claudebot");
    ...
    expect(onNewChatInProject).toHaveBeenCalledWith("/Users/me/claudebot", "Photos");
    ...
    expect(onRequestRenameProject).toHaveBeenCalledWith("/Users/me/claudebot", "Photos");
  });
