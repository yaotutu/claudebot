 ...
 * - **Production**: off unless ``localStorage.setItem('claudebot_debug_ws','1')`` (or ``true``).
 * - **Silence anywhere**: ``localStorage.setItem('claudebot_debug_ws','0')`` (or ``false`` / ``off``).
...
function wsInboundDebugEnabled(): boolean {
  ...
  try {
    ...
    const raw = ls?.getItem("claudebot_debug_ws")?.trim().toLowerCase() ?? "";
...
export class ClaudebotClient {
  ...
  constructor(private options: ClaudebotClientOptions) {
  ...
  private handleMessage(ev: MessageEvent): void {
    ...
    try {
      ...
      if (wsInboundDebugEnabled()) {
        ...
        console.warn(
          "[claudebot ws inbound] invalid JSON",
    ...
    if (wsInboundDebugEnabled()) {
      console.log("[claudebot ws inbound]", summarizeInboundWsPayload(parsed));
    }
    ...
    if (parsed.event === "runtime_model_updated") {
      this.emitRuntimeModelUpdate(parsed.model_name || null);
      return;
    }
