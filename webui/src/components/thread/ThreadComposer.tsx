...
import type { SendImage, SendOptions } from "@/hooks/useClaudebotStream";
import type {
  CliAppInfo,
  GoalStateWsPayload,
  McpPresetInfo,
...
import { CLAUDE_CODE_BRAND, logoFallbackUrls } from "@/lib/brand-assets";
...
const SLASH_RECENTS_STORAGE_KEY = "claudebot.webui.slashCommandRecents";
const SLASH_RECENTS_LIMIT = 5;
const QUEUED_PROMPTS_STORAGE_PREFIX = "claudebot.webui.composerQueuedGuidance.v1:";
  ...
  return (
    ...
    >
      {goalPanelOpen && canExpandGoal && markdownBody ? (
        <div
          ref={panelRef}
          id="claudebot-goal-panel-root"
          role="dialog"
          aria-modal="false"
          aria-labelledby="claudebot-goal-panel-title"
          tabIndex={-1}
          className={cn(
        ...
        >
          ...
          <div
            id="claudebot-goal-panel-scroll"
      ...
      >
        ...
        {canExpandGoal ? (
          <button
            ...
            aria-controls={goalPanelOpen ? "claudebot-goal-panel-root" : undefined}
  ...
  const filteredSlashCommands = useMemo<SlashPaletteCommand[]>(() => {
    ...
    const withDetails = visibleSlashCommands
      ...
      .map((command) => {
        ...
        if (command.command === "/stop" && isStreaming) {
    ...
    return withDetails
      .slice(0, 8);
  }, [isStreaming, recentSlashCommands, slashQuery, t, visibleSlashCommands]);
...
function ComposerModelBadge({
  ...
  const brand = needsSetup ? null : CLAUDE_CODE_BRAND;
  const [logoIndex, setLogoIndex] = useState(0);
  const logoUrl = brand?.logoUrls[logoIndex];
  const showLogo = !!logoUrl;
  const title = needsSetup ? label : `${label} · Claude Code`;
  const interactive = Boolean(onClick);
  const Container = interactive ? "button" : "span";

  useEffect(() => setLogoIndex(0), [needsSetup]);
  ...
  return (
    ...
    >
      <span
        data-testid={needsSetup ? "composer-model-setup-icon" : "composer-model-logo-claude-code"}
        className={cn(
