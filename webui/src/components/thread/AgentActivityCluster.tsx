...
import { faviconUrls, logoFallbackUrls } from "@/lib/brand-assets";
...
function describeTraceLine(line: string): TraceDescription {
  ...
  if (isClaudeCodeFileReadTool(name)) {
    return {
      kind: "tool",
      label: name,
      detail: previewClaudeCodeToolDetail(args) || previewTraceDetail(args, trimmed),
    };
  }
  if (isClaudeCodeFileEditTool(name)) {
    return {
      kind: "tool",
      label: name,
      detail: previewClaudeCodeToolDetail(args) || previewTraceDetail(args, trimmed),
...
function isClaudeCodeFileReadTool(name: string): boolean {
  return new Set(["Read", "Glob", "Grep", "LS"]).has(name);
}

function isClaudeCodeFileEditTool(name: string): boolean {
  return new Set(["Edit", "Write", "MultiEdit", "NotebookEdit"]).has(name);
}

function isClaudeCodeShellTool(name: string): boolean {
  return name === "Bash";
}

function previewClaudeCodeToolDetail(args: string): string {
  ...
  try {
    ...
    for (const key of ["file_path", "path", "pattern", "glob", "content"]) {
      const value = record[key];
      if (typeof value === "string" && value.trim()) return truncateMiddle(value.trim(), 120);
    }
  } catch {
    return "";
  }
...
function previewMcpArgs(argsObject: unknown): string {
  ...
  const record = argsObject as Record<string, unknown>;
  for (const key of ["file_path", "url", "query", "q", "path", "name", "id", "title", "message", "text", "content"]) {
