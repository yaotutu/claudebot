...
const SIDEBAR_STORAGE_KEY = "claudebot-webui.sidebar";
const COMPLETED_RUNS_STORAGE_KEY = "claudebot-webui.sidebar.completed-runs.v1";
const RESTART_STARTED_KEY = "claudebot-webui.restartStartedAt";
...
function readShellRoute(): ShellRoute {
  ...
  if (path === "/apps") return defaultShellRoute();
  if (path === "/skills") {
...
export default function App() {
      ...
      (async () => {
        ...
        try {
          ...
          const client = new ClaudebotClient({
            ...
            onReauth: async () => {
              try {
                const refreshed = await refreshReadyClient(client);
  ...
  useEffect(() => {
    ...
    const timer = window.setTimeout(async () => {
      try {
        await refreshReadyClient(client);
      } catch (e) {
        const msg = (e as Error).message;
        if (msg.includes("HTTP 401") || msg.includes("HTTP 403")) {
          setState({ status: "auth", failed: true });
...
function Shell({
  ...
  const sidebarFlowWidth = sidebarOpen ? SIDEBAR_WIDTH : SIDEBAR_RAIL_WIDTH;
    ...
    <ThemeProvider theme={theme}>
      <div className="relative h-full w-full overflow-hidden">
        ...
        >
          {showMainSidebar ? (
            <aside
              data-testid="sidebar-flow"
              className={cn(
                "relative z-20 hidden shrink-0 overflow-hidden lg:block",
                "transition-[width] duration-300 ease-out",
              )}
              style={{
                width: sidebarFlowWidth,
              }}
            >
              <div className="absolute inset-y-0 left-0 h-full w-full overflow-hidden bg-sidebar shadow-inner-right">
                <Sidebar
                  {...sidebarProps}
