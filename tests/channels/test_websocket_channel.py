...
from claudebot.channels.websocket import (
    WebSocketChannel,
    WebSocketConfig,
    _is_valid_chat_id,
    _parse_envelope,
...
from claudebot.claude_code.runner import ClaudeCodeRunResult
...
from claudebot.webui.gateway_services import GatewayServices, build_gateway_services
...
from claudebot.webui.settings_api import settings_payload
...
class _FakeClaudeSessions:
    def __init__(self) -> None:
        self.records: dict[str, ClaudeCodeChatRecord] = {}
        self.saved: list[ClaudeCodeChatRecord] = []
    ...
    def get_or_create(
        ...
        *,
        ...
        if chat_id not in self.records:
            self.records[chat_id] = ClaudeCodeChatRecord(
                chat_id=chat_id,
...
class _FakeClaudeRunner:
    def __init__(
        self,
        events: list[ClaudeCodeEvent] | None = None,
        final_text: str = "ok",
        *,
    ...
    async def run_turn(
        ...
        self.calls.append({"record": record, "prompt": prompt})
        ...
        return ClaudeCodeRunResult(
            ...
            session_id=record.claude_session_id or "claude-session",
            exit_code=0,
        )
...
def _ch(bus: Any, **kw: Any) -> WebSocketChannel:
    ...
    gateway = build_gateway_services(
        ...
        claude_runner=kw.get("claude_runner", _FakeClaudeRunner()),
        claude_sessions=kw.get("claude_sessions", _FakeClaudeSessions()),
        claude_config=kw.get("claude_config", ClaudeCodeConfig()),
    )
    return WebSocketChannel(cfg, bus, gateway=gateway)


async def _drain_webui_turn(channel: WebSocketChannel, chat_id: str = "chat-1") -> None:
    for _ in range(50):
        task = channel._active_webui_turns.get(chat_id)
        if task is None:
            return
        if task.done():
            await task
...
def isolate_webui_workspace_state(tmp_path, monkeypatch) -> None:
    monkeypatch.setattr("claudebot.config.paths.get_data_dir", lambda: tmp_path)
    monkeypatch.setattr(
        "claudebot.webui.workspaces.get_webui_dir",
...
def test_issue_route_secret_matches_bearer_and_header() -> None:
    ...
    x_headers = Headers([("X-Claudebot-Auth", "my-secret")])
...
@pytest.mark.asyncio
async def test_token_issue_route_requires_secret_when_static_token_configured(
    bus: MagicMock,
) -> None:
    port = 29882
    channel = _ch(
...
async def test_webui_message_envelope_marks_inbound_metadata(bus: MagicMock) -> None:
    from claudebot.webui.transcript import read_transcript_lines
    ...
    await _drain_webui_turn(channel)

    bus.publish_inbound.assert_not_awaited()
    runner = channel.gateway.claude_runner
    assert runner.calls[0]["record"].chat_id == "websocket:chat-1"
    assert runner.calls[0]["prompt"] == "hello"
    lines = read_transcript_lines("websocket:chat-1")
    assert [line["event"] for line in lines] == ["user", "message", "turn_end"]
    assert lines[0] == {
        "event": "user",
        "chat_id": "chat-1",
        "text": "hello",
...
@pytest.mark.asyncio
async def test_webui_message_streams_claude_events_to_client(bus: MagicMock) -> None:
    fake_runner = _FakeClaudeRunner(
        events=[
            ClaudeCodeEvent(kind=EventKind.TEXT_DELTA, text="hel"),
            ClaudeCodeEvent(
            ...
            ClaudeCodeEvent(kind=EventKind.TURN_DONE, text="hello"),
        ],
        final_text="hello",
    )
    channel = _ch(bus, claude_runner=fake_runner)
    conn = AsyncMock()
    conn.remote_address = ("127.0.0.1", 50123)
    ...
    bus.publish_inbound.assert_not_awaited()
    frames = [json.loads(call.args[0]) for call in conn.send.await_args_list]
    assert any(frame["event"] == "delta" and frame["text"] == "hel" for frame in frames)
    tool_frame = next(
        frame
        for frame in frames
        if frame["event"] == "message" and frame.get("kind") == "tool_hint"
    )
    assert tool_frame["tool_events"][0]["name"] == "Read"
    assert any(frame["event"] == "stream_end" for frame in frames)
    assert not any(
        frame["event"] == "message"
        and frame.get("kind") != "progress"
        and frame.get("text") == "hello"
        for frame in frames
    )
    assert any(frame["event"] == "turn_end" for frame in frames)


@pytest.mark.asyncio
async def test_webui_message_does_not_duplicate_turn_done_final_text(bus: MagicMock) -> None:
    fake_runner = _FakeClaudeRunner(
        events=[ClaudeCodeEvent(kind=EventKind.TURN_DONE, text="final answer")],
    ...
    answer_frames = [
        ...
        and frame.get("kind") != "progress"
        and frame.get("text") == "final answer"
    ]
...
async def test_webui_message_envelope_persists_user_transcript_for_refresh(
    ...
    from claudebot.webui.transcript import build_webui_thread_response, read_transcript_lines
    ...
    for _ in range(20):
        lines = read_transcript_lines("websocket:chat-1")
        if [line["event"] for line in lines] == ["user", "message", "turn_end"]:
            break
        await asyncio.sleep(0.01)
    lines = read_transcript_lines("websocket:chat-1")
    assert [line["event"] for line in lines] == ["user", "message", "turn_end"]
    ...
    assert [message["content"] for message in body["messages"]] == ["hello", "ok"]
...
async def test_webui_stop_control_message_is_not_persisted_as_user_bubble(
    ...
    runner = channel.gateway.claude_runner
    assert runner.calls == []
    assert all(line["event"] != "user" for line in read_transcript_lines("websocket:chat-1"))


@pytest.mark.asyncio
async def test_webui_stop_interrupts_active_claude_turn(
    bus: MagicMock,
    tmp_path,
    monkeypatch,
) -> None:
    ...
    monkeypatch.setattr("claudebot.config.paths.get_data_dir", lambda: tmp_path)
    runner = _FakeClaudeRunner(wait_for_interrupt=True)
    channel = _ch(bus, claude_runner=runner)
    conn = AsyncMock()
    conn.remote_address = ("127.0.0.1", 50123)
    ...
    for _ in range(20):
        if runner.interrupt_calls:
            break
        await asyncio.sleep(0.01)
    assert runner.interrupt_calls == 1
    assert all(line.get("text") != "/stop" for line in read_transcript_lines("websocket:chat-1"))
...
async def test_webui_user_transcript_append_failure_does_not_block_inbound(
    ...
    monkeypatch.setattr("claudebot.webui.transcript.append_transcript_object", fail_append)
    channel = _ch(bus)
    conn = AsyncMock()
    conn.remote_address = ("127.0.0.1", 50123)
...
async def test_webui_message_scope_inherits_persisted_session_scope(
    ...
    await _drain_webui_turn(channel, "chat-scope")

    bus.publish_inbound.assert_not_awaited()
    runner = channel.gateway.claude_runner
    record = runner.calls[0]["record"]
    assert Path(record.workspace_path) == project.resolve()
...
async def test_send_broadcasts_runtime_model_updates() -> None:
    ...
    publish_runtime_model_update(bus, "glm-cn/glm-5.1")
    await channel.send(bus.outbound.get_nowait())

    payload = json.loads(mock_ws.send.call_args[0][0])
    assert payload["event"] == "runtime_model_updated"
    assert payload["model_name"] == "glm-cn/glm-5.1"
...
async def test_send_stages_external_media_as_signed_url(monkeypatch, tmp_path) -> None:
    ...
    monkeypatch.setattr("claudebot.channels.websocket.get_media_dir", fake_media_dir)
    monkeypatch.setattr("claudebot.webui.media_gateway.get_media_dir", fake_media_dir)
    channel = WebSocketChannel(
        {"enabled": True, "allowFrom": ["*"]}, bus, gateway=_basic_handler(bus)
    )
    mock_ws = AsyncMock()
...
@pytest.mark.asyncio
async def test_settings_api_returns_gateway_subset_and_updates_network_safety(
    bus: MagicMock,
    monkeypatch,
    tmp_path,
) -> None:
    ...
    monkeypatch.setattr("claudebot.config.loader._current_config_path", config_path)
    ...
    try:
        ...
        assert "secret-token" not in settings.text
        ...
        provider_update = await _http_get(
            "http://127.0.0.1:"
        ...
        assert provider_update.status_code == 404
        ...
        assert network_safety_body["restart_required_sections"] == ["runtime"]
...
async def test_commands_api_returns_slash_command_metadata(bus: MagicMock) -> None:
    ...
    try:
        ...
        assert commands["/help"]["title"] == "Show help"
...
@pytest.mark.asyncio
async def test_bootstrap_omits_legacy_runtime_metadata(bus: MagicMock) -> None:
    port = 29893
    channel = WebSocketChannel(
        {
            "enabled": True,
            ...
            "port": port,
            "path": "/ws",
            "tokenIssueSecret": "bootstrap-secret",
            "websocketRequiresToken": True,
        },
        bus,
        gateway=_basic_handler(bus),
    ...
    try:
        response = await _http_get(
            ...
            headers={"X-Claudebot-Auth": "bootstrap-secret"},
        )
        assert response.status_code == 200
        body = response.json()
        assert "runtime_surface" not in body
        assert "runtime_capabilities" not in body
...
def test_settings_payload_reports_workspace_sandbox(monkeypatch, tmp_path) -> None:
    ...
    save_config(config, config_path)
    monkeypatch.setattr("claudebot.config.loader._current_config_path", config_path)
    monkeypatch.setenv("CLAUDEBOT_SANDBOX_ENFORCED", "macos_app_sandbox")
...
def test_settings_payload_omits_legacy_runtime_metadata(monkeypatch, tmp_path) -> None:
    config_path = tmp_path / "config.json"
    save_config(Config(), config_path)
    monkeypatch.setattr("claudebot.config.loader._current_config_path", config_path)

    body = settings_payload(restart_required_sections=["runtime"])
...
def test_handle_webui_thread_get_returns_json(tmp_path, monkeypatch) -> None:
    ...
    from claudebot.webui.transcript import append_transcript_object
...
def test_handle_file_preview_returns_workspace_file(tmp_path) -> None:
    ...
    workspace = tmp_path / "workspace"
    source = workspace / "claudebot" / "agent" / "hook.py"
    ...
    enc = quote(key, safe="")
    path = quote("claudebot/agent/hook.py:12", safe="")
    req = Request(
    ...
    assert resp.status_code == 200
    body = json.loads(resp.body.decode())
    assert body["display_path"] == "claudebot/agent/hook.py"
...
def test_file_preview_normalizes_windows_file_url() -> None:
    from claudebot.webui.file_preview import _clean_preview_path
