...
from claudebot.webui.media_api import (
    b64url_decode,
    b64url_encode,
)
...
def test_sign_media_path_rejects_paths_outside_media_root(bus: MagicMock, tmp_path: Path) -> None:
    ...
    with patch("claudebot.webui.media_gateway.get_media_dir", return_value=media):
...
def test_local_markdown_image_is_staged_and_rewritten(
    ...
    with patch("claudebot.webui.media_gateway.get_media_dir", side_effect=_fake_media_dir(media)):
...
def test_local_markdown_video_is_staged_and_rewritten(
    ...
    (workspace / "claudebot-intro.mp4").write_bytes(video_bytes)
    media = tmp_path / "media"
    channel = _ch(bus, workspace_path=workspace, port=0)

    with patch("claudebot.webui.media_gateway.get_media_dir", side_effect=_fake_media_dir(media)):
        rewritten = channel.gateway.media.rewrite_local_markdown_images(
            "The result:\n![claudebot-intro.mp4](claudebot-intro.mp4)"
        )

    assert "![claudebot-intro.mp4](/api/media/" in rewritten
    staged = list((media / "websocket").iterdir())
    assert len(staged) == 1
