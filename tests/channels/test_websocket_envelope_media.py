...
class _FakeClaudeRunner:
    ...
    async def run_turn(
        ...
        return ClaudeCodeRunResult(final_text="ok", session_id="sid", exit_code=0)
...
def _make_channel() -> WebSocketChannel:
    ...
    gateway = build_gateway_services(
        ...
        claude_sessions=_FakeClaudeSessions(),
    )
...
def test_max_message_bytes_default_supports_multi_image_frame() -> None:
    ...
    from claudebot.channels.websocket import WebSocketConfig
...
async def test_message_forwards_normalized_cli_app_attachments() -> None:
    ...
    task = channel._active_webui_turns["abc123"]
    await asyncio.wait_for(task, timeout=1)
    runner = channel.gateway.claude_runner
    assert runner.calls[0]["record"].chat_id == "websocket:abc123"
    assert runner.calls[0]["prompt"] == "please use @drawio"
    lines = read_transcript_lines("websocket:abc123")
    assert lines[0]["cli_apps"] == [
        {
            "name": "drawio",
            "display_name": "Draw.io",
            "category": "diagram",
...
async def test_message_with_single_image_forwards_saved_path(tmp_path) -> None:
    ...
    with patch("claudebot.channels.websocket.get_media_dir", return_value=tmp_path):
        await channel._dispatch_envelope(mock_conn, "client-1", envelope)
