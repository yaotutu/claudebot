...
from claudebot.cron.types import CronJob, CronPayload, CronSchedule
from claudebot.session.manager import Session, SessionManager
...
async def test_webui_skills_route_requires_token_and_hides_paths(
    ...
    workspace_skill = tmp_path / ".claude" / "skills" / "workspace-skill"
    workspace_skill.mkdir(parents=True)
    (workspace_skill / "SKILL.md").write_text(
        "---\nname: workspace-skill\ndescription: Workspace skill.\n---\n",
        encoding="utf-8",
    )
    unavailable_skill = tmp_path / ".claude" / "skills" / "zz-unavailable-skill"
    ...
    (unavailable_skill / "SKILL.md").write_text(
        "\n".join(
            [
                ...
                "    requires:",
                "      bins:",
                "        - definitely-missing-claudebot-skill-cli",
                "      env:",
                "        - DEFINITELY_MISSING_CLAUDEBOT_SKILL_ENV",
                "---",
    ...
    try:
        ...
        assert unavailable["unavailable_reason"] == (
            "CLI: definitely-missing-claudebot-skill-cli, "
            "ENV: DEFINITELY_MISSING_CLAUDEBOT_SKILL_ENV"
        )
        ...
        assert detail_body["requirements"] == {
            "bins": ["definitely-missing-claudebot-skill-cli"],
            "env": ["DEFINITELY_MISSING_CLAUDEBOT_SKILL_ENV"],
            "missing_bins": ["definitely-missing-claudebot-skill-cli"],
            "missing_env": ["DEFINITELY_MISSING_CLAUDEBOT_SKILL_ENV"],
        }
...
@pytest.mark.asyncio
async def test_legacy_cli_apps_settings_routes_are_removed(
    bus: MagicMock,
    tmp_path: Path,
) -> None:
...
@pytest.mark.asyncio
async def test_claude_code_settings_routes_require_token_and_accept_json_updates(
    bus: MagicMock,
    tmp_path: Path,
    ...
    def _update_claude_code_settings(data: dict[str, Any]) -> dict[str, Any]:
        updates.append(data)
        return {
            **settings_payload,
    ...
    monkeypatch.setattr(
        "claudebot.webui.settings_routes.claude_code_settings_payload",
        lambda: settings_payload,
    )
    monkeypatch.setattr(
        "claudebot.webui.settings_routes.claude_code_health_payload",
        lambda: health_payload,
    )
    monkeypatch.setattr(
        "claudebot.webui.settings_routes.update_claude_code_settings",
        _update_claude_code_settings,
    )

    channel = _ch(bus, session_manager=_seed_session(tmp_path), port=29921)
    channel.gateway.tokens.api_tokens["live"] = time.monotonic() + 60
    ...
    deny = await channel.gateway.http.settings_routes.dispatch(
        _RouteReq("/api/settings/claude-code"),
        "/api/settings/claude-code",
    )
    assert deny is not None
    assert deny.status_code == 401

    settings = await channel.gateway.http.settings_routes.dispatch(
        _RouteReq("/api/settings/claude-code", headers=auth),
        "/api/settings/claude-code",
    )
    assert settings is not None
    assert settings.status_code == 200
    assert json.loads(settings.body)["claudeCode"]["model"] == "glm-cn/glm-5.1"

    health = await channel.gateway.http.settings_routes.dispatch(
        _RouteReq("/api/settings/claude-code/health", headers=auth),
        "/api/settings/claude-code/health",
    )
    assert health is not None
    assert health.status_code == 200
    assert json.loads(health.body)["health"]["lastError"] == "not reachable"
    ...
    updated = await channel.gateway.http.settings_routes.dispatch(
        _RouteReq(
    ...
    assert updates == [update_body]
    assert body["requires_restart"] is True
    assert body["restart_required_sections"] == ["runtime"]


@pytest.mark.asyncio
async def test_legacy_mcp_preset_settings_routes_are_removed(
    bus: MagicMock,
    tmp_path: Path,
) -> None:
    ...
    try:
        ...
        enabled = await _http_get(
            ...
            headers={
                **auth,
                "X-Claudebot-MCP-Values": json.dumps({"browserbase_api_key": "bb_live_secret"}),
            },
        )
...
def test_wildcard_ipv6_with_secret_is_valid(bus: MagicMock) -> None:
    ...
    resp = channel.gateway.http._handle_bootstrap(_REMOTE, _FakeReq({"X-Claudebot-Auth": "s3cret"}))
    assert resp.status_code == 200
...
def test_bootstrap_ws_url_uses_forwarded_https_host(bus: MagicMock) -> None:
    ...
    resp = channel.gateway.http._handle_bootstrap(
        _LOCAL,
        _FakeReq({"Host": "claudebot.example", "X-Forwarded-Proto": "https"}),
    )
    assert resp.status_code == 200
    body = json.loads(resp.body)
    assert body["ws_url"] == "wss://claudebot.example/"
...
def test_bootstrap_prefers_runtime_model_name(
    ...
    monkeypatch.setattr(
        "claudebot.webui.ws_http._default_model_name_from_config",
        lambda: "from-disk",
    )
...
def test_bootstrap_accepts_x_claudebot_auth_header(bus: MagicMock) -> None:
    channel = _ch(bus, host="0.0.0.0", tokenIssueSecret="s3cret")
    resp = channel.gateway.http._handle_bootstrap(_REMOTE, _FakeReq({"X-Claudebot-Auth": "s3cret"}))
    assert resp.status_code == 200
