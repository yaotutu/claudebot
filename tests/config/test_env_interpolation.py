...
from claudebot.config.loader import (
    _resolve_env_vars,
    load_config,
...
class TestResolveConfig:
    def test_resolves_env_vars_in_config(self, tmp_path, monkeypatch):
        ...
        config_path.write_text(
            json.dumps({"claudeCode": {"apiKey": "${TEST_API_KEY}"}}),
            encoding="utf-8",
        )

        raw = load_config(config_path)
        assert raw.claude_code.api_key.get_secret_value() == "${TEST_API_KEY}"

        resolved = resolve_config_env_vars(raw)
        assert resolved.claude_code.api_key.get_secret_value() == "resolved-key"
    ...
    def test_resolves_claude_code_env_refs_with_unrelated_templates(self, tmp_path, monkeypatch):
        monkeypatch.setenv("TEST_API_KEY", "resolved-key")
        config_path = tmp_path / "config.json"
        config_path.write_text(
            json.dumps(
                {
                    "claudeCode": {"apiKey": "${TEST_API_KEY}"},
                    "channels": {"telegram": {"token": "${TEST_API_KEY}"}},
                }
            ),
            encoding="utf-8",
        )
