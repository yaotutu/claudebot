"""Tests for claudebot.security.network — SSRF protection and internal URL detection."""
...
from claudebot.security.network import (
...
def test_blocks_private_ipv4(ip: str, label: str):
    with patch("claudebot.security.network.socket.getaddrinfo", _fake_resolve("evil.com", [ip])):
...
def test_blocks_ipv6_loopback():
    ...
    with patch("claudebot.security.network.socket.getaddrinfo", _resolver):
...
def test_blocks_ipv6_mapped_loopback():
    ...
    with patch(
        "claudebot.security.network.socket.getaddrinfo",
...
def test_allows_normal_https():
    with patch(
        ...
        ok, err = validate_url_target("https://github.com/your-org/claudebot")
        assert ok
...
def test_detects_wget_localhost():
    with patch(
        "claudebot.security.network.socket.getaddrinfo", _fake_resolve("localhost", ["127.0.0.1"])
    ):
...
def test_loopback_exception_allows_literal_localhost_only():
    with patch(
        ...
        assert not contains_internal_url("curl http://localhost:18790/", allow_loopback=True)


def test_loopback_exception_rejects_public_name_resolving_to_loopback():
    with patch(
        "claudebot.security.network.socket.getaddrinfo", _fake_resolve("example.com", ["127.0.0.1"])
    ):
        assert contains_internal_url("curl http://example.com:18790/", allow_loopback=True)
...
def test_whitelist_does_not_affect_other_blocked():
    ...
    try:
        with patch(
            "claudebot.security.network.socket.getaddrinfo", _fake_resolve("evil.com", ["10.0.0.1"])
        ):
